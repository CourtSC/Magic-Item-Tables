...
def ddbScraper():
    ...
    while nextPage:
        res = session.get(
            "https://www.dndbeyond.com/magic-items?page=" + str(page), headers=headers
        )
        ...
        itemSubType = soup.select(
            ".row.item-type .subtype"
        )  # Need to strip whitespace.
        itemAttunement = soup.select(
            ".row.requires-attunement"
        )  # Need to strip whitespace.
        ...
        for i in range(len(itemName)):
            if itemAttunement[i].getText().strip() == "Required":
                if itemSubType[i].getText().strip():
                    data.append(
                        (
                            itemName[i].getText().strip(),
                            itemRarity[i].getText().strip(),
                            itemType[i].getText().strip(),
                            itemSubType[i].getText().strip(),
                            itemAttunement[i].getText().strip(),
                            itemNotes[i].getText().strip(),
                        )
                    )
                else:
                    data.append(
    ...
    with open("magicItems.csv", "w", encoding="UTF-8") as dBase:
